...
function StatusBadge({ status }: { status: string }) {
  if (status === "available") return <Badge className="bg-emerald-500 hover:bg-emerald-600 text-white" data-testid="badge-status">Available</Badge>;
  if (status === "dispatched") return <Badge className="bg-amber-500 hover:bg-amber-600 text-white" data-testid="badge-status">Dispatched</Badge>;
  return <Badge variant="secondary" data-testid="badge-status">Maintenance</Badge>;
}
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">Real-time status of all ambulances in the network.</p>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{ambulance.type.toUpperCase()} Life Support</p>
                      </div>
                    </div>
      ...
      {!isLoading && ambulances?.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-muted-foreground">No ambulances found for the selected filters.</p>
        </div>
      )}
    </div>
  );
